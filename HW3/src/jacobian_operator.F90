...
#include <finclude/petsc.h90>
  ...
  type, public :: jacobian_operator_type
  ...
  end type jacobian_operator_type
...
!==============================================================================
  ...
  subroutine init_J_operator(this)
    ...
    ! get preallocation
    call preallocate_jacobian_matrix(this)
...
!===============================================================================
  ...
  subroutine preallocate_jacobian_matrix(this)
    ...
    if (rank == n_procs - 1) then
      this % d_nnz(row_start:row_end-1) = loss%d_nnz
      this % o_nnz(row_start:row_end-1) = loss%o_nnz
    else
      this % d_nnz = loss%d_nnz
      this % o_nnz = loss%o_nnz
    end if
...
!===============================================================================
  ...
  subroutine build_jacobian_matrix(snes,x,jac,jac_prec,flag,ctx,ierr)
     ...
     if (.not. allocated(cols)) allocate(cols(maxval(loss%d_nnz+loss%o_nnz)))
     if (.not. allocated(vals)) allocate(vals(maxval(loss%d_nnz+loss%o_nnz)))
     cols = 0
     vals = 0.0_8
     ...
     call VecCreateMPI(PETSC_COMM_WORLD,loss%localn,PETSC_DECIDE,phi,ierr)
     ...
     call MatAXPY(loss%oper,-1.0_8*lambda,prod%oper,SUBSET_NONZERO_PATTERN,ierr)

     ! create tmp petsc vector for source
     call VecCreateMPI(PETSC_COMM_WORLD,loss%localn,PETSC_DECIDE,source,ierr)

     ! perform math (-F*phi --> source)
     call MatMult(prod%oper,phi,source,ierr)
