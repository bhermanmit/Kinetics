module snes_solver
 ...
 &                                  build_prod_matrix,destroy_F_operator
  use jacobian_operator, only: jacobian_operator_type,init_J_operator,         &
...
#include <finclude/petsc.h90>
  ...
  type(jacobian_operator_type) :: jac_prec
...
PetscViewer :: viewer
    ...
    call VecCreateMPI(PETSC_COMM_WORLD,loss%localn,PETSC_DECIDE,rphi,ierr)
    ...
    call VecCreateMPI(PETSC_COMM_WORLD,loss%localn,PETSC_DECIDE,phiM,ierr)

    ! calculate flux part of residual vector
    call MatMult(loss%oper,phi,phiM,ierr)
    call MatMult(prod%oper,phi,rphi,ierr)
