...
def gnuplot_file(results):
  ...
  outstr = """
set terminal pdf 
set output "partB_spaceerror.pdf"
set key left top
set log x
set log y
set grid
...
plot '-' using 1:2 with linespoint linewidth 3.0 title "Max Core Power", '-' using 1:2 with linespoint linewidth 3.0 title "End Core Power"
  ...
  with open("gnuplot_space.dat",'w') as f:
    f.write(outstr)
...
def process_all():
  ...
  results = []
  temp = result('./mesh_1/output.h5',1.0)
  results.append(temp)
  temp = result('./mesh_2/output.h5',2.0)
  results.append(temp)
  temp = result('./mesh_5/output.h5',5.0)
  results.append(temp)
  temp = result('./mesh_10/output.h5',10.0)
  results.append(temp)
