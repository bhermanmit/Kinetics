...
\begin{frame}{Autonomous Forms of Runge Kutta}
  ...
  \end{equation}}
  \only<2->{\begin{equation}
    \nonumber \mathbf{k}_i = h\mathbf{f}\left(\mathbf{y}_n + \sum_{j=1}^{{\color{green!75!black}s}}a_{ij}\mathbf{k}_j\right) 
  \end{equation}}
...
\begin{frame}{Convergence rate for PK with feedback}
  \begin{center}
  \includegraphics[scale=0.7]{./figs/order_pk_feedback.pdf}
  \end{center}
    \begin{itemize}
