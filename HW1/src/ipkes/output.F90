...
!-module options
  ...
  public :: title, plot_results, write_input, write_results, write_message,    &
            header, write_physics, write_output
...
!===============================================================================
  ...
  subroutine title()
    ...
    write(UNIT=OUTPUT_UNIT, FMT='(/6(A/))') &
'       __  .__   __. ____    ____      .______    __  ___  _______ ' ,&
'      |  | |  \ |  | \   \  /   /      |   _  \  |  |/  / |   ____|' ,&
'      |  | |   \|  |  \   \/   / ______|  |_)  | |  "  /  |  |__   ' ,&
'      |  | |  . `  |   \      / |______|   ___/  |    <   |   __|  ' ,&
'      |  | |  |\   |    \    /         |  |      |  .  \  |  |____ ' ,&
...
!---begin execution
    ...
    ! echo input
    write(OUTPUT_UNIT, fmt='(A,T30,I0)') 'Number of time steps: ', sum(ipke % nt)
    write(OUTPUT_UNIT, fmt='(/,"Time (s)",T20,"Rho ($)")')
    write(OUTPUT_UNIT, fmt='(  "--------",T20,"-------")')
    do i = 1, ipke % npts
      write(OUTPUT_UNIT, fmt='(1PE9.3,T20,1PE9.3)') ipke % t(i), ipke % power(i)
    end do
